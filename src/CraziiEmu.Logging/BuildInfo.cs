...
public static class BuildInfo
{
    ...
    static BuildInfo()
    {
        ...
        CommitSha = Normalize(metadata.GetValueOrDefault("CraziiEmu.BuildSha"));
        if (CommitSha is { Length: > 7 })
        {
            CommitSha = CommitSha[..7];
        }

        Branch = Normalize(metadata.GetValueOrDefault("CraziiEmu.BuildBranch"));
        Repository = Normalize(metadata.GetValueOrDefault("CraziiEmu.BuildRepository"));
        Configuration = Normalize(metadata.GetValueOrDefault("CraziiEmu.BuildConfiguration"));

        var serverUrl = Normalize(metadata.GetValueOrDefault("CraziiEmu.BuildServerUrl"));
        var runId = Normalize(metadata.GetValueOrDefault("CraziiEmu.BuildRunId"));
        ...
        var eventName = Normalize(metadata.GetValueOrDefault("CraziiEmu.BuildEventName"));
        var gitRef = Normalize(metadata.GetValueOrDefault("CraziiEmu.BuildRef"));
    ...
    public static string Banner
    {
        get
        {
            ...
            var header = $"CraziiEmu {version} — {ProjectUrl}";
    ...
    private static Dictionary<string, string> ReadMetadata()
    {
        ...
        foreach (var assembly in assemblies)
        {
            ...
            {
                if (attribute.Key.StartsWith("CraziiEmu.Build", StringComparison.Ordinal) &&
