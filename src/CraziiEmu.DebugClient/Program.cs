...
internal static class ClientProgram
{
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        await using (connection)
        {
            ...
            else
            {
                if (!quiet)
                {
                    Console.WriteLine($"Connected to CraziiEmu debug server at {host}:{port}.");
    ...
    {
        Console.WriteLine("CraziiEmu.DebugClient — live debugger client for the CraziiEmu debug server.");
        Console.WriteLine();
        Console.WriteLine("Usage: CraziiEmu.DebugClient [host:port] [--exec \"<command>\"]... [--quiet]");
