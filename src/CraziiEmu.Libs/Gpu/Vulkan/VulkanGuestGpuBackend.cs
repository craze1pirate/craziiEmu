...
namespace CraziiEmu.Libs.Gpu.Vulkan;

/// <summary>
/// Vulkan backend for the guest-GPU seam: SPIR-V codegen via
/// CraziiEmu.ShaderCompiler.Vulkan, rendering via a thin adapter over the existing
...
internal sealed class VulkanGuestGpuBackend : IGuestGpuBackend
{
    ...
    public bool TrySubmitGuestImage(
        ...
        long flipArg)
    {
        return VulkanVideoPresenter.TrySubmitGuestImage(address, width, height, pitchInPixel, videoOutHandle, flipArg);
    }
    ...
    public bool TrySubmitOrderedGuestImageFlip(
        ...
        uint pitchInPixel)
    {
        return VulkanVideoPresenter.TrySubmitOrderedGuestImageFlip(
            videoOutHandle,
            displayBufferIndex,
            flipArg,
            address,
    ...
    public void AttachGuestMemory(CraziiEmu.HLE.ICpuMemory memory) =>
