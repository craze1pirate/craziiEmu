...
using CraziiEmu.ShaderCompiler.Metal;

namespace CraziiEmu.Libs.Gpu.Metal;
...
internal static partial class MetalVideoPresenter
{
    ...
    private static void SyncDrawableSizeToLayer()
    {
        ...
        // every Metal title. CRAZIIEMU_METAL_CAP_DRAWABLE=1 enables the long-edge
        // cap; CRAZIIEMU_METAL_FULL_DRAWABLE=1 remains a no-op when the cap is off.
        if (string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_METAL_CAP_DRAWABLE"),
                "1",
                StringComparison.Ordinal) &&
            !string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_METAL_FULL_DRAWABLE"),
                "1",
                StringComparison.Ordinal))
        {
    ...
    {
        pipeline = 0;
        var dbg = Environment.GetEnvironmentVariable("CRAZIIEMU_METAL_DBG");
