...
{
    public static ArrayPool<byte> GuestDataPool { get; } = ArrayPool<byte>.Shared;
    ...
    {
        if (pixelSpirv.Length == 0 || depthTarget.Width == 0 || depthTarget.Height == 0)
        {
            return;
        }

        var target = new VulkanGuestRenderTarget(
            depthTarget.Address,
            depthTarget.Width,
            depthTarget.Height,
            depthTarget.GuestFormat,
            0,
            1);
    ...
    private static long _perfDraws;
    private static double _perfDrawMs;
    private static long _perfPipelines;
    private static long _perfSpirvCompiles;
    ...
    public static int SubmitOrderedGuestAction(Action action, string name)
    {
        lock (_gate)
        {
            if (_closed) return -1;
    ...
    public static (long draws, double drawMs, long pipelines, long spirvCompiles) ReadAndResetPerfCounters()
    {
        var draws = Interlocked.Exchange(ref _perfDraws, 0);
        var drawMs = Interlocked.Exchange(ref _perfDrawMs, 0);
        var pipelines = Interlocked.Exchange(ref _perfPipelines, 0);
        var spirv = Interlocked.Exchange(ref _perfSpirvCompiles, 0);
        return (draws, drawMs, pipelines, spirv);
    }
    ...
    public static bool TrySubmitGuestImageBlit(
        ...
        uint sourceHeight,
        uint sourceFormat,
        uint sourceNumberType = 0,
        ulong destinationAddress = 0,
        uint destinationWidth = 0,
        uint destinationHeight = 0,
        uint destinationFormat = 0,
        uint destinationNumberType = 0)
    {
        if (sourceAddress == 0 ||
    ...
    private sealed class Presenter : IDisposable
    {
        ...
        private void CreateInstance()
        {
            ...
            var enableValidation = Environment.GetEnvironmentVariable("CraziiEmu_VK_VALIDATION") == "1";
            ...
            try
            {
                ...
                try
                {
                    ...
                    if (validationLayerName is not null)
                    {
                        Console.Error.WriteLine("[LOADER][WARN] Validation layer enabled but ExtDebugUtils is not supported locally.");
                    }
                }
                finally
                {
            ...
            VulkanGuestMemoryBuffer guestBuffer)
        {
            ...
            SetDebugName(
                ObjectType.Buffer,
                buffer.Handle,
                $"CraziiEmu global 0x{guestBuffer.BaseAddress:X16} {guestBuffer.Data.Length}b");
        ...
        private void ExecuteComputeDispatch(VulkanComputeGuestDispatch work)
        {
            ...
            if (AddressListContains("CraziiEmu_SKIP_COMPUTE_CS", work.ShaderAddress))
            {
                TraceVulkanShader(
        ...
        private void ExecuteOffscreenDraw(VulkanOffscreenGuestDraw work)
        {
            ...
            try
            {
                ...
                resources = CreateTranslatedDrawResources(
                    ...
                    extent);
                resources.DebugName =
                    $"CraziiEmu offscreen rt=0x{work.Target.Address:X16} " +
        ...
        {
            if (!string.Equals(Environment.GetEnvironmentVariable("CraziiEmu_LOG_AGC"), "1", StringComparison.Ordinal) &&
                !string.Equals(Environment.GetEnvironmentVariable("CraziiEmu_LOG_AGC_SHADER"), "1", StringComparison.Ordinal))
            {
                return;
            }
