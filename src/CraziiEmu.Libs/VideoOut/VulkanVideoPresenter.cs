...
internal sealed record VulkanUnmapGpuRange(
    ulong Address,
    ulong Size);
...
internal static unsafe class VulkanVideoPresenter
{
    ...
    private static readonly int _maxPendingGuestWorkItems =
        int.TryParse(
            Environment.GetEnvironmentVariable("CRAZIIEMU_PENDING_GUEST_WORK_ITEMS"),
    ...
    private static readonly ulong _maxPendingGuestWorkBytes =
        (ulong.TryParse(
             Environment.GetEnvironmentVariable("CRAZIIEMU_PENDING_GUEST_WORK_MB"),
             out var pendingGuestWorkMb) && pendingGuestWorkMb > 0
            ? pendingGuestWorkMb
            : 256UL) * 1024UL * 1024UL;
    private static readonly int _maxGuestWorkPerRender =
        int.TryParse(
            Environment.GetEnvironmentVariable("CRAZIIEMU_MAX_GUEST_WORK_PER_RENDER"),
    ...
    // queued for the next frame. CRAZIIEMU_RENDER_WORK_BUDGET_MS overrides
    ...
    private static readonly long _renderWorkBudgetTicks =
        (long.TryParse(
             Environment.GetEnvironmentVariable("CRAZIIEMU_RENDER_WORK_BUDGET_MS"),
    ...
    private static readonly int _guestWorkFollowupWaitMs =
        int.TryParse(
            Environment.GetEnvironmentVariable("CRAZIIEMU_RENDER_FOLLOWUP_WAIT_MS"),
            out var followupWaitMs) && followupWaitMs >= 0
            ? followupWaitMs
            : 2;
    private static readonly long _guestWorkFollowupBudgetTicks =
        (long.TryParse(
             Environment.GetEnvironmentVariable("CRAZIIEMU_RENDER_FOLLOWUP_BUDGET_MS"),
    ...
    // CRAZIIEMU_FRAME_WAIT_BUDGET_MS overrides; default 8ms on macOS. The
    ...
    private static readonly ulong _frameSlotWaitBudgetNs =
        ulong.TryParse(
            Environment.GetEnvironmentVariable("CRAZIIEMU_FRAME_WAIT_BUDGET_MS"),
    ...
    // CRAZIIEMU_FENCE_WAIT_TIMEOUT_MS overrides; default 3s.
    private static readonly ulong _guestFenceWaitTimeoutNs =
        ulong.TryParse(Environment.GetEnvironmentVariable("CRAZIIEMU_FENCE_WAIT_TIMEOUT_MS"), out var fenceMs) && fenceMs > 0
            ? fenceMs * 1_000_000UL
            : 3_000_000_000UL;
    ...
    // CRAZIIEMU_SUBMISSION_CAPACITY_WAIT_MS overrides; default 100ms; 0 restores
    // the full blocking wait.
    private static readonly ulong _submissionCapacityWaitNs =
        ulong.TryParse(Environment.GetEnvironmentVariable("CRAZIIEMU_SUBMISSION_CAPACITY_WAIT_MS"), out var capMs)
            ? capMs * 1_000_000UL
            : 100_000_000UL;
    ...
    // CRAZIIEMU_PENDING_GUEST_SYNC_ITEMS (default 8x payload item cap).
    private static readonly int _maxPendingGuestSyncItems =
        int.TryParse(
            Environment.GetEnvironmentVariable("CRAZIIEMU_PENDING_GUEST_SYNC_ITEMS"),
    ...
    private static readonly bool _skipAllCompute =
        Environment.GetEnvironmentVariable("CRAZIIEMU_SKIP_ALL_COMPUTE") == "1";
    ...
    private static readonly uint _skipTallComputeZ =
        uint.TryParse(Environment.GetEnvironmentVariable("CRAZIIEMU_SKIP_TALL_COMPUTE_Z"), out var z)
            ? z
            : 0;
    ...
    private static readonly bool _traceGuestImageEvents =
        string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_DRAWS"),
            "1",
            StringComparison.Ordinal) ||
        string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_GUEST_IMAGE_EVENTS"),
            "1",
            StringComparison.Ordinal);
    private static readonly bool _traceGuestWorkCompletion =
        string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_GUEST_WORK_COMPLETION"),
            "1",
            StringComparison.Ordinal);
    private static readonly bool _traceOrderedActionLatency =
        string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_ORDERED_ACTION_LATENCY"),
            "1",
            StringComparison.Ordinal);
    private static readonly bool _traceGlobalWritebackTiming =
        string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_GLOBAL_WRITEBACK_TIMING"),
            "1",
            StringComparison.Ordinal);
    ...
    private static bool ShouldTraceGuestImageSubmissionsForDiagnostics()
    {
        return string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_GUEST_IMAGES"),
            "1",
            StringComparison.Ordinal);
    }
    ...
    {
        var mode = Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_GUEST_IMAGES");
        ...
        lock (_gate)
        {
            ...
            var enqueued = EnqueueGuestWorkLocked(
                new VulkanOrderedGuestFlip(
                    ...
                    height,
                    pitchInPixel)) > 0;
            CraziiEmu.Libs.Diagnostics.LoadProgressDiagnostics.TraceOrderedFlipEnqueue(
                videoOutHandle,
                displayBufferIndex,
                address,
                version,
    ...
    private static volatile CraziiEmu.HLE.ICpuMemory? _guestMemory;

    internal static void AttachGuestMemory(CraziiEmu.HLE.ICpuMemory memory) =>
        _guestMemory = memory;
        ...
        ulong scopeByteCount = ulong.MaxValue)
    {
        _ = scopeAddress;
        if (scopeByteCount == 0 ||
            !CraziiEmu.HLE.GuestImageWriteTracker.Enabled)
        {
            return;
        }
    ...
    public static void UnmapGpuRange(ulong address, ulong size)
    {
        ...
        foreach (var key in _cachedTextureIdentities.Keys)
        {
            if (key.Address >= address && key.Address < end)
            {
        ...
        lock (_gate)
        {
            ...
            {
                _availableGuestImages.Remove(addr);
                _guestImageExtents.Remove(addr);
                _guestImageWorkSequences.Remove(addr);
                _cpuBackedUploadGenerations.Remove(addr);
                _untrackedGuestImageContentProbes.Remove(addr);
                _pendingGuestImageInitialData.Remove(addr);
            }

            var matchingUploads = _pendingGuestImageUploads.Keys
                .Where(pair => pair.Address >= address && pair.Address < end)
                .ToArray();
            foreach (var pair in matchingUploads)
            {
                _pendingGuestImageUploads.Remove(pair);
            }

            EnqueueGuestWorkLocked(new VulkanUnmapGpuRange(address, size));
        }
    }
        ...
        uint height = 0)
    {
        ...
        lock (_gate)
        {
            ...
            if (width > 0 && height > 0)
            {
                var bytesPerPixel = guestFormat == 9 ? 4u : 4u;
                var byteCount = (ulong)width * height * bytesPerPixel;
                _guestImageExtents[address] = (width, height, byteCount);
                if (ShouldTrackGuestImageWrites(byteCount))
                {
                    CraziiEmu.HLE.GuestImageWriteTracker.Track(
                        address,
                        byteCount,
                        CurrentGuestWorkSequenceForDiagnostics,
                        "vulkan.display-buffer");
                }
            }
        }
    }
        ...
        uint numberType)
    {
        ...
        lock (_gate)
        {
            ...
            if (_cpuBackedUploadGenerations.TryGetValue(address, out var uploadedGeneration) &&
                CraziiEmu.HLE.GuestImageWriteTracker.TryGetWriteGeneration(
                    address,
    ...
    private static ulong ComputeSparseGuestContentProbe(
        CraziiEmu.HLE.ICpuMemory memory,
        ulong address,
        ulong byteCount)
    {
    ...
    private static void StartPresenterLocked()
    {
        ...
        {
            IsBackground = true,
            Name = "CraziiEmu Vulkan VideoOut",
        };
        _thread.Start();
    }
    ...
    private static long EnqueueGuestWorkLocked(object work)
    {
        ...
        // (CRAZIIEMU_PENDING_GUEST_WORK_MB).
    ...
    {
        CraziiEmu.HLE.GuestImageWriteTracker.FlushPendingDiagnostics();
        lock (_gate)
        {
            if (_traceGuestWorkCompletion)
            {
    ...
    private sealed class Presenter : IDisposable
    {
        ...
        private static readonly bool _gpuDetileEnabled = !string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_GPU_DETILE"), "0", StringComparison.Ordinal);
        private static readonly bool _gpuDetileLog = string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_GPU_DETILE"), "1", StringComparison.Ordinal);
        ...
        private readonly Queue<(GuestImageResource Image, ulong RetireTimeline)>
            _deferredGuestImageDestroys = new();
        private readonly Queue<(GuestDepthResource Depth, ulong RetireTimeline)>
            _deferredGuestDepthDestroys = new();
        ...
        {
            public string DebugName = "CraziiEmu translated";
        ...
        {
            var value = Environment.GetEnvironmentVariable("CRAZIIEMU_RENDERDOC_WAIT");
        ...
        private static string BuildComputeDebugName(VulkanComputeGuestDispatch dispatch)
        {
            ...
            return storage is null
                ? $"CraziiEmu compute cs=0x{dispatch.ShaderAddress:X16} " +
        ...
        private static string GuestImageDebugName(GuestRenderTarget target, Format format) =>
            $"CraziiEmu guest 0x{target.Address:X16} {target.Width}x{target.Height} " +
            $"fmt{target.Format}/{format}";

        private static string TextureDebugName(GuestDrawTexture texture, Format format) =>
            $"CraziiEmu texture 0x{texture.Address:X16} {texture.Width}x{texture.Height} " +
        ...
        {
            var applicationName = (byte*)SilkMarshal.StringToPtr("CraziiEmu");
            ...
            try
            {
                ...
                byte** extensions;
                uint extensionCount;
                if (_window?.VkSurface is { } glfwSurface)
                {
                    extensions = glfwSurface.GetRequiredExtensions(out extensionCount);
                }
                else
                {
                    throw new InvalidOperationException("GLFW/Silk.NET did not provide Vulkan surface extensions.");
                }
                ...
                {
                    Console.Error.WriteLine("[LOADER][WARN] CRAZIIEMU_VK_VALIDATION=1 but VK_LAYER_KHRONOS_validation not found (Vulkan SDK installed?).");
                }
                ...
                try
                {
                    ...
                    if (validationLayerName is not null && _vk.TryGetInstanceExtension(_instance, out ExtDebugUtils debugUtils))
                    {
                        ...
                        Console.Error.WriteLine("[LOADER][INFO] Vulkan Validation Layers active (CRAZIIEMU_VK_VALIDATION=1).");
                    }
                }
                finally
                {
            ...
            void* userData)
        {
                ...
                message.Contains("vkCreateShaderModule", StringComparison.Ordinal))
            {
                ...
                var dumpHint = dumpPath is null
                    ? "Set CRAZIIEMU_SHADER_SPIRV_DUMP_DIR to a directory to "
        ...
        private void SelectPhysicalDevice()
        {
            ...
            var deviceOverride = Environment.GetEnvironmentVariable("CRAZIIEMU_VK_DEVICE");
        ...
        private void LoadComputeDeviceLimits()
        {
            ...
            _minStorageBufferOffsetAlignment = Math.Max(
                ...
                1UL);
            _maxStorageBufferRange = properties.Limits.MaxStorageBufferRange != 0
        ...
        private void CreateSwapchain()
        {
            ...
            {
                HostHdrMode.On => true,
                HostHdrMode.Auto => guestHdrRequested,
                _ => false,
            };
        ...
        private void RecordOverlayBlit(uint imageIndex, int frameSlot)
        {
            ...
            CraziiEmu.Libs.Metrics.MetricsManager.RefreshStatsIfDue(0, 0);
        ...
        private void ExecuteOrderedGuestFlip(VulkanOrderedGuestFlip work)
        {
            ...
            if (source is null)
            {
                uint width = work.Width > 0 ? work.Width : 1920;
                uint height = work.Height > 0 ? work.Height : 1080;
                uint formatCode = 56;
                uint numberType = 0;
                Format vkFormat = Format.B8G8R8A8Unorm;

                if (VideoOutExports.TryGetDisplayBufferInfo(work.VideoOutHandle, work.DisplayBufferIndex, out var displayBuffer))
                {
                    if (displayBuffer.Width > 0) width = displayBuffer.Width;
                    if (displayBuffer.Height > 0) height = displayBuffer.Height;
                    if (VideoOutExports.IsHdrPixelFormat(displayBuffer.PixelFormat))
                    {
                        formatCode = 9;
                        vkFormat = Format.A2R10G10B10UnormPack32;
                    }
                    else
                    {
                        formatCode = 56;
                ...
                else if (_availableGuestImages.TryGetValue(work.Address, out var guestFmt))
                {
                    if (guestFmt == 9)
                    {
                        formatCode = 9;
                ...
                var target = new GuestRenderTarget(
                    ...
                    formatCode,
                    numberType);
                source = GetOrCreateGuestImage(target, vkFormat);
                source.IsCpuBacked = true;
            }
            ...
            if (!source.Initialized)
            {
                var expectedByteCount = GetVulkanImageByteCount(
                    ...
                    source.Height,
                    source.Depth);
                if (expectedByteCount > 0 && expectedByteCount <= 128UL * 1024UL * 1024UL && _guestMemory is not null)
                {
                    var pixels = new byte[expectedByteCount];
                    if (_guestMemory.TryRead(source.Address, pixels))
                    {
                        UploadGuestImageInitialData(source, pixels);
                        FlushBatchedGuestCommands();
                    }
                }
            ...
            else if (source.IsCpuBacked && _guestMemory is not null)
            {
                if (!CraziiEmu.HLE.GuestImageWriteTracker.Enabled || CraziiEmu.HLE.GuestImageWriteTracker.ConsumeDirty(source.Address))
                {
                    var expectedByteCount = GetVulkanImageByteCount(
                        source.Format,
                        source.Width,
                        source.Height,
                        source.Depth);
                    if (expectedByteCount > 0 && expectedByteCount <= 128UL * 1024UL * 1024UL)
                    {
                        var pixels = new byte[expectedByteCount];
                        if (_guestMemory.TryRead(source.Address, pixels))
                        {
        ...
        private void ExecuteUnmapGpuRange(VulkanUnmapGpuRange unmap)
        {
            var start = unmap.Address;
            ...
            if (imagesToEvict != null)
            {
                foreach (var imgAddress in imagesToEvict)
                {
            ...
            if (variantsToEvict != null)
            {
                foreach (var key in variantsToEvict)
                {
            ...
            if (depthToEvict != null)
            {
                foreach (var key in depthToEvict)
                {
            ...
            List<TextureContentIdentity>? texturesToEvict = null;
            foreach (var (identity, _) in _textureCache)
            {
                if (identity.Address >= start && identity.Address < end)
                {
                    texturesToEvict ??= [];
                    texturesToEvict.Add(identity);
                }
            }
            if (texturesToEvict != null)
            {
                foreach (var identity in texturesToEvict)
                {
                    if (_textureCache.Remove(identity, out var texture))
                    {
                        _deferredTextureDestroys.Enqueue((texture, _submitTimeline));
                    }
                }
            }

            List<GuestBufferAllocation>? bufferOverlaps = null;
            foreach (var allocation in _guestBufferAllocations)
            {
                if (allocation.BaseAddress < end && start < allocation.BaseAddress + allocation.Size)
                {
                    bufferOverlaps ??= [];
                    bufferOverlaps.Add(allocation);
                }
            }
            if (bufferOverlaps != null)
            {
                foreach (var overlap in bufferOverlaps)
                {
                    _guestBufferAllocations.Remove(overlap);
                    if (overlap.Mapped != 0)
                    {
                        _vk.UnmapMemory(_device, overlap.Memory);
                        overlap.Mapped = 0;
                    }

                    if (overlap.LastUseTimeline > _completedTimeline)
                    {
                        _batchRetireBuffers.Add((overlap.Buffer, overlap.Memory));
                    }
                    else
                    {
                        _vk.DestroyBuffer(_device, overlap.Buffer, null);
                        _vk.FreeMemory(_device, overlap.Memory, null);
                    }
                }
            }
        ...
        private void ProcessDeferredTextureDestroys()
        {
            ...
            while (_deferredGuestImageDestroys.TryPeek(out var variantEntry) &&
                   variantEntry.RetireTimeline <= _completedTimeline)
            {
                _deferredGuestImageDestroys.Dequeue();
                DestroyGuestImage(variantEntry.Image);
            }

            while (_deferredGuestDepthDestroys.TryPeek(out var depthEntry) &&
                   depthEntry.RetireTimeline <= _completedTimeline)
            {
                _deferredGuestDepthDestroys.Dequeue();
                DestroyGuestDepth(depthEntry.Depth);
            }
        }
        ...
        private ShaderModule CreateShaderModule(byte[] code)
        {
            ...
            var dumpDirectory = Environment.GetEnvironmentVariable("CRAZIIEMU_SHADER_SPIRV_DUMP_DIR");
            ...
            GuestDepthResource? feedbackDepth = null)
        {
            ...
            var forceSolidFragment = forceTitleSolidFragment ||
                ...
                AnyTargetAddressMatches(
                    feedbackTargets,
                    "CRAZIIEMU_FORCE_SOLID_FRAGMENT_TARGETS");
            ...
            var forceAttributeFragment =
                ...
                AnyTargetAddressMatches(
                    feedbackTargets,
                    "CRAZIIEMU_FORCE_ATTRIBUTE_FRAGMENT_TARGETS");
            ...
            ShaderStageFlags stageFlags)
        {
            ...
            fixed (WriteDescriptorSet* writePointer = writes)
            {
                ...
                if (globalBufferCount != 0)
                {
                    ...
                    {
                        var bufferRange = resources.GlobalMemoryBuffers[index].Size;
                        if (_maxStorageBufferRange != 0 && bufferRange > _maxStorageBufferRange)
                        {
                            bufferRange = _maxStorageBufferRange;
                        }
            ...
            try
            {
                ...
                fixed (VertexInputAttributeDescription* vertexAttributePointerBase = vertexAttributeDescriptions)
                {
                    ...
                    SetDebugName(
                        ObjectType.Pipeline,
                        pipeline.Handle,
                        $"CraziiEmu graphics ps={fragmentSpirv.Length}b attrs={resources.Textures.Length}");
                }
            }
            finally
            {
            ...
            byte[] computeSpirv)
        {
            ...
            try
            {
                ...
                SetDebugName(
                    ObjectType.Pipeline,
                    pipeline.Handle,
                    $"CraziiEmu compute cs={computeSpirv.Length}b");
            ...
            foreach (var depth in _guestDepthImages.Values)
            {
                ...
                if (!depth.SampleViews.TryGetValue(texture.DstSelect, out var view))
                {
                    ...
                    SetDebugName(
                        ...
                        view.Handle,
                        $"CraziiEmu guest depth sample 0x{depth.Address:X16} " +
        ...
        {
            var syncEnabled = CraziiEmu.HLE.GuestImageWriteTracker.Enabled;
            ...
            if (extents is not null)
            {
                ...
                {
                    if (!CraziiEmu.HLE.GuestImageWriteTracker.ConsumeDirty(address))
                    {
                        continue;
                    }
            ...
            if (_textureCache.Count == 0)
            {
                if (dirtyAddresses is not null)
                {
                    ...
                    {
                        CraziiEmu.HLE.GuestImageWriteTracker.Rearm(address);
                    }
                }
        ...
        private TextureResource CreateStorageScratchResource(GuestDrawTexture texture)
        {
            ...
            SetDebugName(ObjectType.Image, image.Handle, $"CraziiEmu scratch storage {width}x{height} {vkFormat}");
            SetDebugName(ObjectType.ImageView, view.Handle, $"CraziiEmu scratch storage {width}x{height} {vkFormat} view");
        ...
        private TextureResource CreateTextureResource(GuestDrawTexture texture)
        {
            ...
            else
            {
                ...
                if (AddressListContains("CRAZIIEMU_FORCE_WHITE_TEXTURE_TARGETS", texture.Address))
                {
                ...
                !_guestImages.ContainsKey(texture.Address))
            {
                ...
                if (_guestImages.Count >= 1024)
                {
                    ulong evictAddress = 0;
                    GuestImageResource? evictResource = null;
                    foreach (var (addr, candidate) in _guestImages)
                    {
                        if (candidate.IsCpuBacked && addr != texture.Address)
                        {
                            evictAddress = addr;
                            evictResource = candidate;
                            break;
                        }
                    }
                    ...
                    if (evictResource is not null)
                    {
                        ...
                        lock (_gate)
                        {
                            _availableGuestImages.Remove(evictAddress);
                            _cpuBackedUploadGenerations.Remove(evictAddress);
                            _guestImageExtents.Remove(evictAddress);
                        }
                        CraziiEmu.HLE.GuestImageWriteTracker.Untrack(evictAddress);
                    }
                }
            ...
            GuestImageResource source)
        {
            ...
            try
            {
                ...
                var debugName =
                    $"CraziiEmu feedback 0x{source.Address:X16} " +
            ...
            GuestDepthResource source)
        {
            ...
            try
            {
                ...
                SetDebugName(
                    ObjectType.Image,
                    image.Handle,
                    $"CraziiEmu depth feedback 0x{source.Address:X16} image");
                SetDebugName(
                    ObjectType.ImageView,
                    view.Handle,
                    $"CraziiEmu depth feedback 0x{source.Address:X16} view");
                return new TextureResource
                {
                    Address = texture.Address,
                    Image = image,
        ...
        {
            if (!string.Equals(
                    Environment.GetEnvironmentVariable("CRAZIIEMU_DUMP_TEXTURES"),
                    "1",
                    StringComparison.Ordinal) ||
                texture.IsFallback ||
                texture.IsStorage ||
            ...
            Format format)
        {
            if (!_traceGuestImageAddressFilterEnabled ||
                !AddressListContains("CRAZIIEMU_TRACE_GUEST_IMAGE_ADDRS", texture.Address) ||
            ...
            var directory =
                Environment.GetEnvironmentVariable("CRAZIIEMU_GUEST_IMAGE_DUMP_DIR");
            ...
            ulong end)
        {
            ...
            SetDebugName(
                ObjectType.Buffer,
                buffer.Handle,
                $"CraziiEmu guest VA 0x{start:X16}-0x{end:X16}");
            ...
            GuestVertexBuffer guestBuffer)
        {
            ...
            if (_setDebugUtilsObjectName is not null)
            {
                SetDebugName(
                    ObjectType.Buffer,
                    buffer.Handle,
                    $"CraziiEmu vertex loc{guestBuffer.Location} " +
        ...
        private static readonly float ViewportDebugEpsilon = float.TryParse(
            Environment.GetEnvironmentVariable("CRAZIIEMU_VIEWPORT_EPSILON"),
        ...
        private void ExecuteComputeDispatchCore(VulkanComputeGuestDispatch work)
        {
            ...
            if (_skipAllCompute ||
                AddressListContains("CRAZIIEMU_SKIP_COMPUTE_CS", work.ShaderAddress) ||
        ...
        private void ExecuteOffscreenDrawCore(VulkanOffscreenGuestDraw work)
        {
            ...
            try
            {
                ...
                resources.DebugName =
                    $"CraziiEmu offscreen mrt={targets.Length} " +
            ...
            uint depth = 1)
        {
            ...
            if (_guestImages.TryGetValue(target.Address, out var existing))
            {
                ...
                var variantKey = new GuestImageVariantKey(
                    existing.Address,
                ...
                var matchingKeys = _guestImageVariants.Keys.Where(k => k.Address == existing.Address).ToList();
                if (matchingKeys.Count >= 2)
                {
                    foreach (var oldKey in matchingKeys)
                    {
                ...
                while (_guestImageVariants.Count >= 32)
                {
                ...
                _guestImageVariants.Add(variantKey, existing);
                ...
                CraziiEmu.HLE.GuestImageWriteTracker.Untrack(target.Address);
            }
        ...
        private GuestDepthResource GetOrCreateGuestDepth(GuestDepthTarget target)
        {
            ...
            SetDebugName(
                ObjectType.Image,
                image.Handle,
                $"CraziiEmu guest depth 0x{target.Address:X16} {target.Width}x{target.Height}");
            SetDebugName(
                ObjectType.ImageView,
                view.Handle,
                $"CraziiEmu guest depth view 0x{target.Address:X16}");

            while (_guestDepthImages.Count >= 16)
            {
            ...
            GuestDepthResource depth)
        {
            ...
            var name = $"CraziiEmu color 0x{color.Address:X16} depth 0x{depth.Address:X16}";
        ...
        private static readonly bool _realFormatConversionEnabled = string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_ENABLE_REAL_FORMAT_CONVERSION"),
            "1",
            StringComparison.Ordinal);
            ...
            bool arrayedView = false)
        {
            ...
            SetDebugName(
                ObjectType.ImageView,
                view.Handle,
                $"CraziiEmu guest 0x{resource.Address:X16} alias {format} mip{mipLevel}+{levelCount}");
            TraceVulkanShader(
        ...
        private void RenderCore()
        {
                ...
                try
                {
                    ...
                    switch (work)
                    {
                        ...
                        case VulkanOrderedGuestFlipWait flipWait:
                            ...
                            break;
                        case VulkanUnmapGpuRange unmapRange:
                            ExecuteUnmapGpuRange(unmapRange);
                            break;
                    }
                }
                finally
            ...
            if (drawableSizeChanged || _swapchainRecreateDeferred)
            {
                RecreateSwapchainResources(
                    drawableSizeChanged ? "drawable resize" : "restored window",
            ...
            if (!tookPresentation)
            {
                ...
                if (CraziiEmu.Libs.Diagnostics.LoadProgressDiagnostics.IsActive ||
                    ShouldTracePresentedGuestImageContentsForDiagnostics())
                {
                    ...
                    CraziiEmu.Libs.Diagnostics.LoadProgressDiagnostics.TracePresentNotTaken(
                        _presentedSequence,
                        hasPendingPresentation);
                    CraziiEmu.Libs.Diagnostics.LoadProgressDiagnostics.TraceGpuWaitSnapshot();
            ...
            CraziiEmu.Libs.Diagnostics.LoadProgressDiagnostics.TracePresentTaken(
            ...
            else if (presentation.GuestImageAddress != 0)
            {
                ...
                {
                    uint width = presentation.Width > 0 ? presentation.Width : 1920;
                    uint height = presentation.Height > 0 ? presentation.Height : 1080;
                    uint formatCode = 56;
                    uint numberType = 0;
                    Format vkFormat = Format.B8G8R8A8Unorm;

                    if (VideoOutExports.TryGetDisplayBufferInfo(presentation.VideoOutHandle, 0, out var displayBuffer))
                    {
                        if (displayBuffer.Width > 0) width = displayBuffer.Width;
                        if (displayBuffer.Height > 0) height = displayBuffer.Height;
                        if (VideoOutExports.IsHdrPixelFormat(displayBuffer.PixelFormat))
                    ...
                    else if (_availableGuestImages.TryGetValue(presentation.GuestImageAddress, out var guestFmt))
                    {
                        if (guestFmt == 9)
                        {
                            formatCode = 9;
                    ...
                    var target = new GuestRenderTarget(
                        ...
                        formatCode,
                        numberType);
                    presentedGuestImage = GetOrCreateGuestImage(target, vkFormat);
                    presentedGuestImage.IsCpuBacked = true;
                }

                if (presentedGuestImage is not null && !presentedGuestImage.Initialized)
                {
                    var expectedByteCount = GetVulkanImageByteCount(
                        presentedGuestImage.Format,
                        presentedGuestImage.Width,
                        presentedGuestImage.Height,
                        presentedGuestImage.Depth);
                    if (expectedByteCount > 0 && expectedByteCount <= 128UL * 1024UL * 1024UL && _guestMemory is not null)
                    {
                        var guestPixels = new byte[expectedByteCount];
                        if (_guestMemory.TryRead(presentedGuestImage.Address, guestPixels))
                        {
                            UploadGuestImageInitialData(presentedGuestImage, guestPixels);
                            FlushBatchedGuestCommands();
                        }
                    }

                    if (!presentedGuestImage.Initialized)
                    {
                        presentedGuestImage.Initialized = true;
                    }
                }
                else if (presentedGuestImage is not null && presentedGuestImage.IsCpuBacked && _guestMemory is not null)
                {
                    if (!CraziiEmu.HLE.GuestImageWriteTracker.Enabled || CraziiEmu.HLE.GuestImageWriteTracker.ConsumeDirty(presentedGuestImage.Address))
                    {
                        var expectedByteCount = GetVulkanImageByteCount(
                            presentedGuestImage.Format,
                            presentedGuestImage.Width,
        ...
        {
            BeginDebugLabel(_commandBuffer, "CraziiEmu swapchain draw");
            RecordGlobalBufferVisibilityBarrier(
                _commandBuffer,
                resources,
                PipelineStageFlags.VertexShaderBit |
            ...
            ulong shaderAddress = 0)
        {
            ...
            if (_traceGuestImageShaderFilterEnabled &&
                !AddressListContains(
                    "CRAZIIEMU_TRACE_GUEST_IMAGE_SHADER_ADDRS",
                    shaderAddress))
            {
                return false;
            }
        ...
        // CRAZIIEMU_TRACE_GUEST_IMAGES=every:N[@M] — read back 1280x720+ guest
        ...
        private static readonly bool _vulkanValidationEnabled =
            string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_VK_VALIDATION"),
                "1",
                StringComparison.Ordinal);
        ...
        private static readonly bool _vulkanDebugUtilsEnabled =
            ...
            string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_VK_DEBUG_LABELS"),
                "1",
                StringComparison.Ordinal);
        ...
        private static readonly bool _traceVulkanResourcesEnabled =
            string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_VK_RESOURCES"),
                "1",
                StringComparison.Ordinal);
        private static readonly bool _traceVulkanShaderEnabled =
            string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_AGC"),
                "1",
                StringComparison.Ordinal) ||
            string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_AGC_SHADER"),
                "1",
                StringComparison.Ordinal);
        private static readonly bool _traceDepthInitialization =
            string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_DEPTH_INIT"),
                "1",
                StringComparison.Ordinal);
        private static readonly long _traceGuestImageOccurrence =
            long.TryParse(
                Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_GUEST_IMAGE_OCCURRENCE"),
        ...
        private static readonly bool _forceFullscreenPipeline =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_FULLSCREEN_PIPELINE") == "1";
        private static readonly bool _forceFullscreenVertex =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_FULLSCREEN_VERTEX") == "1";
        private static readonly bool _forceTitleFullscreenVertex =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_TITLE_FULLSCREEN_VERTEX") == "1";
        private static readonly bool _forceDefaultRasterState =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_DEFAULT_RASTER_STATE") == "1";
        private static readonly bool _forceTitleDefaultRasterState =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_TITLE_DEFAULT_RASTER_STATE") == "1";
        private static readonly bool _forceTitleSolidFragment =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_TITLE_SOLID_FRAGMENT") == "1";
        private static readonly bool _forceSolidFragment =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_SOLID_FRAGMENT") == "1";
        private static readonly uint? _forceAttributeFragmentLocation =
            uint.TryParse(
                Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_ATTRIBUTE_FRAGMENT"),
        ...
        private static readonly string? _fixedFragmentDumpPath =
            Environment.GetEnvironmentVariable("CRAZIIEMU_DUMP_FIXED_SOLID_FRAGMENT");
        private static readonly bool _forceTitleDefaultBlend =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_TITLE_DEFAULT_BLEND") == "1";
        private static readonly bool _forceTitleDefaultViewportScissor =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_TITLE_DEFAULT_VIEWPORT_SCISSOR") == "1";
        private static readonly bool _forceTitleDisableCull =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_TITLE_DISABLE_CULL") == "1";
        private static readonly bool _forceTitleDisableDepth =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_TITLE_DISABLE_DEPTH") == "1";
        private static readonly bool _traceTitleState =
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_TITLE_STATE") == "1";
        private static readonly bool _forceTitleVertexColorWhite =
            Environment.GetEnvironmentVariable("CRAZIIEMU_FORCE_TITLE_VERTEX_COLOR_WHITE") == "1";
        private static readonly bool _chunkedDrawsEnabled =
            Environment.GetEnvironmentVariable("CRAZIIEMU_ENABLE_CHUNKED_DRAWS") == "1";
        private static readonly string? _traceGuestWritesMode =
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_GUEST_WRITES");
        private static readonly long _traceGuestWriteOrdinal =
            long.TryParse(
                Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_GUEST_WRITE_ORDINAL"),
        ...
        private bool ShouldTraceAddressedPresentedGuestImage(GuestImageResource image)
        {
            if (!AddressListContains(
                    "CRAZIIEMU_TRACE_PRESENTED_GUEST_IMAGE_ADDRS",
                    image.Address))
            {
                return false;
            }
            ...
            out ImageView view)
        {
            ...
            SetDebugName(ObjectType.Image, image.Handle, $"CraziiEmu Bink2 {planeName} image");
            SetDebugName(ObjectType.ImageView, view.Handle, $"CraziiEmu Bink2 {planeName} view");
        }
        ...
        private bool TryGetPresentEncodeImage(out Image encodeImage)
        {
            ...
            if (_presentEncodeImage.Handle == 0)
            {
                ...
                SetDebugName(
                    ...
                    "CraziiEmu present sRGB-encode image");
            }
            ...
            GuestImageResource source)
        {
            ...
            BeginDebugLabel(
                _commandBuffer,
                $"CraziiEmu present image 0x{source.Address:X16}");
        ...
        private void TraceSwapchainReadback()
        {
            ...
            try
            {
                ...
                var dumpDir = Environment.GetEnvironmentVariable("CRAZIIEMU_GUEST_IMAGE_DUMP_DIR");
                if (!string.IsNullOrWhiteSpace(dumpDir))
                {
					...
					if (string.Equals(
							Environment.GetEnvironmentVariable("CRAZIIEMU_GUEST_IMAGE_DUMP_CONTINUOUS"),
							"1",
							StringComparison.Ordinal))
					{
        ...
        {
            var raw = Environment.GetEnvironmentVariable("CRAZIIEMU_SWAPCHAIN_DUMP_EVERY");
        ...
        private void DisposeVulkan()
        {
            ...
            while (_deferredGuestImageDestroys.TryDequeue(out var deferredImage))
            {
                DestroyGuestImage(deferredImage.Image);
            }
            ...
            long sequence)
        {
            ...
            return work switch
            {
                ...
                VulkanUnmapGpuRange unmap =>
                    $"unmap_gpu_range addr=0x{unmap.Address:X16} size=0x{unmap.Size:X} {queuePart}",
