...
/// proprietary SDK and CraziiEmu must neither bundle it nor depend on its ABI.
/// </summary>
internal static class Bink2MovieBridge
{
    ...
    {
        var configured = Environment.GetEnvironmentVariable("CRAZIIEMU_BINK_MODE");
        ...
        if (!string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("CRAZIIEMU_BINK2_BRIDGE")) ||
    ...
    private static NativeAdapter? GetAdapterLocked()
    {
        ...
        if (!_availabilityReported)
        {
            ...
            Console.Error.WriteLine(
                "[LOADER][INFO] Bink2 bridge unavailable; install the licensed adapter and set CRAZIIEMU_BINK2_BRIDGE.");
        }
    ...
    {
        var configured = Environment.GetEnvironmentVariable("CRAZIIEMU_BINK2_BRIDGE");
