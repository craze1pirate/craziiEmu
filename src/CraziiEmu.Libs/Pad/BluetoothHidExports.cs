...
// success and let the Pad path provide input (CRAZIIEMU_BTHID_UNAVAILABLE=1 fails instead).
public static class BluetoothHidExports
{
    ...
    private static readonly bool _reportUnavailable = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_BTHID_UNAVAILABLE"),
        "1",
    ...
    // CRAZIIEMU_BTHID_FIRE_CALLBACK=1 enables; CRAZIIEMU_BTHID_EVENT_CODE and
    // CRAZIIEMU_BTHID_EVENT_SIZE (default 0 / 256) shape the synthetic event.
    private static readonly bool _fireCallback = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_BTHID_FIRE_CALLBACK"),
        "1",
        StringComparison.Ordinal);
    ...
    public static int BluetoothHidRegisterCallback(CpuContext ctx)
    {
        ...
        // CRAZIIEMU_BTHID_CB_FAIL=nf -> NOT_FOUND, =ni -> NOT_IMPLEMENTED.
        var mode = Environment.GetEnvironmentVariable("CRAZIIEMU_BTHID_CB_FAIL");
    ...
    private static void FireEnumerationComplete(CpuContext ctx)
    {
        ...
        var eventCode = ParseEnvUInt64("CRAZIIEMU_BTHID_EVENT_CODE", 0);
        var eventSize = (int)ParseEnvUInt64("CRAZIIEMU_BTHID_EVENT_SIZE", 256);
        if (eventSize < 1)
        {
            eventSize = 256;
        }
