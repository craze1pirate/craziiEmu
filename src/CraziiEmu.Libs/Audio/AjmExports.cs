...
public static class AjmExports
{
    ...
    public static int AjmInitialize(CpuContext ctx)
    {
        ...
        if (string.Equals(Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_AJM"), "1", StringComparison.Ordinal))
        {
    ...
    public static int AjmMemoryRegister(CpuContext ctx)
    {
        ...
        // AJM hardware requires explicit registration. CraziiEmu decodes from
    ...
    /// <summary>
    /// Records the gapless-decode window for a looping voice. CraziiEmu decodes
        ...
        ulong countOrSize)
    {
        if (!split ||
            ...
            !string.Equals(Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_AJM"), "1", StringComparison.Ordinal))
        {
            return;
        }
