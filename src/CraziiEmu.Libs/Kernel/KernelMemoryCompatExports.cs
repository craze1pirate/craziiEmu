...
public static partial class KernelMemoryCompatExports
{
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("CraziiEmu_LOG_WIDE_PRINTF"), "1", StringComparison.Ordinal))
        {
            return;
        }
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("CraziiEmu_LOG_WIDE_PRINTF_ARGS"), "1", StringComparison.Ordinal))
        {
            return false;
        }

        var filter = Environment.GetEnvironmentVariable("CraziiEmu_LOG_WIDE_PRINTF_FILTER");
    ...
    private static void TraceLibcAllocation(
        ...
        int? errorCode = null)
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("CraziiEmu_LOG_LIBC_ALLOC"), "1", StringComparison.Ordinal))
        {
            return;
        }
    ...
    private static string? ResolveApp0Root()
    {
        ...
        var configured = Environment.GetEnvironmentVariable("CraziiEmu_APP0_DIR");
    ...
    {
        var configuredRoot = Environment.GetEnvironmentVariable("CraziiEmu_DEVLOG_APP_DIR");
        string root;
    ...
    private static string ResolveDownload0Root()
    {
        ...
        const string download0VariableName = "CraziiEmu_DOWNLOAD0_DIR";
    ...
    {
        const string hostappVariableName = "CraziiEmu_HOSTAPP_DIR";
    ...
    private static readonly bool _traceDirectMemory = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_DIRECT_MEMORY"), "1", StringComparison.Ordinal);
    ...
    private static bool TryAllocateLibcHeapCore(CpuContext ctx, nuint requestedSize, nuint alignment, bool zeroFill, out ulong address)
    {
        address = 0;
        ...
        if (ctx.Memory is IGuestMemoryAllocator allocator &&
            allocator.TryAllocateGuestMemory((ulong)actualSize, (ulong)alignment, out address))
        {
            lock (_libcAllocGate)
            {
                _libcAllocations[address] = new LibcHeapAllocation((nint)address, actualSize, alignment);
            }

            if (zeroFill)
            {
                ctx.Memory.TryWrite(address, new byte[actualSize]);
            }
            return true;
        }
    ...
    private static bool TryReallocateLibcHeap(CpuContext ctx, ulong existingAddress, ulong requestedSize, out ulong resizedAddress)
    {
        resizedAddress = 0;
        ...
        if (bytesToCopy > 0)
        {
            var buffer = System.Buffers.ArrayPool<byte>.Shared.Rent((int)bytesToCopy);
            try
            {
                var span = buffer.AsSpan(0, (int)bytesToCopy);
                if (ctx.Memory.TryRead(existingAddress, span))
                {
                    ctx.Memory.TryWrite(resizedAddress, span);
                }
            }
            finally
            {
    ...
    private static void FreeLibcHeap(ulong address)
    {
        ...
        lock (_libcAllocGate)
        {
            _libcAllocations.Remove(address);
        }
    }
    ...
    internal static int KernelOpenUnderscore(CpuContext ctx)
    {
        var pathAddress = ctx[CpuRegister.Rdi];
        var flags = unchecked((int)ctx[CpuRegister.Rsi]);
        var mode = unchecked((int)ctx[CpuRegister.Rdx]);
        if (pathAddress == 0 || !TryReadCString(ctx, pathAddress, 1024, out var guestPathBytes))
        {
            return (int)OrbisGen2Result.ORBIS_GEN2_ERROR_INVALID_ARGUMENT;
        }
        var guestPath = Encoding.UTF8.GetString(guestPathBytes);
        ...
        if ((flags & O_DIRECTORY) != 0 || Directory.Exists(hostPath))
        {
            ...
            var dirFd = AllocateGuestFileDescriptor();
            ...
            lock (_fdGate)
            {
                _openDirectories[dirFd] = new OpenDirectory { Path = hostPath, Entries = entries };
            }

            LogOpenTrace($"open dir guest='{guestPath}' host='{hostPath}' fd={dirFd}");
            return dirFd;
        }
        ...
        try
        {
            ...
            var fileMode = ResolveOpenMode(flags, access);
            var stream = new FileStream(hostPath, fileMode, access, FileShare.ReadWrite);
            var fd = AllocateGuestFileDescriptor();
            lock (_fdGate)
            {
                _openFiles[fd] = stream;
            }

            LogOpenTrace($"open file guest='{guestPath}' host='{hostPath}' fd={fd} mode={fileMode} access={access}");
            return fd;
        }
    ...
    internal static int KernelFstat(CpuContext ctx)
    {
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("CraziiEmu_LOG_OPEN"), "1", StringComparison.Ordinal))
        {
            return;
        }
    ...
    {
        if (!string.Equals(Environment.GetEnvironmentVariable("CraziiEmu_LOG_IO"), "1", StringComparison.Ordinal))
        {
            return;
        }

        var filter = Environment.GetEnvironmentVariable("CraziiEmu_LOG_IO_FILTER");
