...
public static partial class AgcExports
{
    ...
    private static readonly bool _strictShaderDescriptors = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_STRICT_SHADER_DESCRIPTORS"),
        "1",
        StringComparison.Ordinal);
    ...
    private static readonly ulong? _traceComputeShaderAddress = ParseOptionalHexAddress(
        Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_COMPUTE_SHADER_ADDRESS"));
    private static readonly ulong? _tracePixelShaderAddress = ParseOptionalHexAddress(
        Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_PIXEL_SHADER_ADDRESS"));
    private static readonly ulong? _traceRenderTargetAddress = ParseOptionalHexAddress(
        Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_RENDER_TARGET_ADDRESS"));
    ...
    private static readonly bool _traceFramePackets = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_FRAME_PACKETS"),
        "1",
        StringComparison.Ordinal);
    private static readonly bool _traceVertexRanges = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_VERTEX_RANGES"),
        "1",
        StringComparison.Ordinal);
    private static readonly bool _compatibilitySubmitCompletionEvent = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_AGC_SUBMIT_COMPLETION_EVENT"),
        "1",
        StringComparison.Ordinal);
    ...
    private static readonly bool _textureCopySkipDisabled = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_NO_TEXTURE_SKIP"),
        "1",
        StringComparison.Ordinal);

    // GPU deswizzle: ship raw tiled bytes + params to the backend instead of
    // detiling on the CPU. On by default; CRAZIIEMU_GPU_DETILE=0 forces the CPU
    ...
    private static readonly bool _gpuDetileEnabled = !string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_GPU_DETILE"),
        "0",
        StringComparison.Ordinal);

    // Diagnostics (CRAZIIEMU_LOG_GPU_DETILE=1): one line per distinct texture tile
    ...
    private static readonly bool _gpuDetileLog = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_GPU_DETILE"),
        "1",
        StringComparison.Ordinal);
        ...
        ulong scopeByteCount = ulong.MaxValue)
    {
        ...
        _ = ctx;
        if (!CraziiEmu.HLE.GuestImageWriteTracker.Enabled || scopeByteCount == 0)
        {
            return;
        }
    ...
    // CRAZIIEMU_GPU_WAIT_MODE=force reverts to the legacy behaviour of faking a
    ...
    /// exists only behind CRAZIIEMU_GPU_WAIT_MODE=force for legacy A/B testing;
        ...
        out string error)
    {
        ...
        if (pixelShaderAddress == 0x0000000500781200 &&
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_TITLE_GLOBALS") == "1")
        {
            TraceAstroTitlePixelGlobals(pixelEvaluation);
        }

        if (pixelShaderAddress == 0x0000000500781200 &&
            Environment.GetEnvironmentVariable("CRAZIIEMU_TRACE_TITLE_GLOBALS_LIVE") == "1")
        {
    ...
    private static readonly bool _fillClearHack = !string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_DISABLE_FILL_CLEAR"),
        "1",
        StringComparison.Ordinal);
    ...
    /// CRAZIIEMU_DISABLE_FILL_CLEAR=1.
    /// </summary>
    ...
    private static readonly bool _bakeScalars = string.Equals(
        Environment.GetEnvironmentVariable("CRAZIIEMU_BAKE_SGPRS"),
        "1",
        StringComparison.Ordinal);
        ...
        out GuestDrawTexture texture)
    {
        ...
        if (isStorage)
        {
            ...
            if (ParseOptionalHexAddress(
                    Environment.GetEnvironmentVariable(
                        "CRAZIIEMU_TRACE_STORAGE_IMAGE_INIT_ADDRESS")) ==
                descriptor.Address)
            {
                Console.Error.WriteLine(
        ...
        if (!_textureCopySkipDisabled &&
            descriptor.Address != 0 &&
            !CraziiEmu.HLE.GuestImageWriteTracker.PeekDirty(descriptor.Address) &&
    ...
    /// CRAZIIEMU_TEXTURE_DUMP_DIR is set, so upload-time content can be
    ...
    private static void DumpTextureSourceIfRequested(
        ...
        byte[] source)
    {
        var directory = Environment.GetEnvironmentVariable("CRAZIIEMU_TEXTURE_DUMP_DIR");
    ...
    /// <summary>
    /// Writes the bytes after detiling when CRAZIIEMU_TEXTURE_LINEAR_DUMP_DIR is
    ...
    private static void DumpLinearTextureIfRequested(
        ...
        byte[] source)
    {
        var directory = Environment.GetEnvironmentVariable("CRAZIIEMU_TEXTURE_LINEAR_DUMP_DIR");
        if (string.IsNullOrWhiteSpace(directory))
        {
            return;
        }
        ...
        Gen5ShaderProgram program)
    {
        if (shader.Payload.Length == 0 ||
            !string.Equals(
                Environment.GetEnvironmentVariable("CRAZIIEMU_DUMP_SPIRV"),
                "1",
                StringComparison.Ordinal))
        {
            return;
    ...
    private static void TraceCreateShader(ulong destinationAddress, ulong headerAddress, ulong codeAddress, string detail)
    {
        ...
        if (isOk &&
            (!string.Equals(Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_AGC"), "1", StringComparison.Ordinal) ||
    ...
    public static int DriverSubmitMultiDcbs(CpuContext ctx)
    {
        ...
        var tracePackets = string.Equals(
            Environment.GetEnvironmentVariable("CRAZIIEMU_LOG_AGC"), "1", StringComparison.Ordinal);
