...
CraziiEmu observes successful guest .bk2 opens and, when a Bink decoder is
...
code (`src/CraziiEmu.Libs/Bink/FfmpegNativeBinkFrameSource.cs`, via the
...
CraziiEmu, and there is no C/C++ code of CraziiEmu's own involved in decoding
-- CraziiEmu.CLI.csproj only downloads a prebuilt release archive.
...
(`src/CraziiEmu.Libs/Bink/FfmpegBinkFrameSource.cs`). CraziiEmu searches
...
`CraziiEmu.CLI.csproj`'s `FfmpegRuntimeTag`, matched to the `FFmpeg.AutoGen`
...
CraziiEmu; both just download a zip once (cached under
...
degrades gracefully: CraziiEmu logs one informational line ("Bink2 bridge
