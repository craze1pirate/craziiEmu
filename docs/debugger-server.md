...
CraziiEmu can expose a **live debug server** so an external process can inspect
and control a running guest over TCP. The server lives in the emulator; the
companion `CraziiEmu.DebugClient` executable is one client, and the wire protocol
is simple enough to script against directly.

This document describes the moving parts and the wire protocol. For day-to-day
client usage, see
[`src/CraziiEmu.DebugClient/DEVELOPER_READ.md`](../src/CraziiEmu.DebugClient/DEVELOPER_READ.md).
...
```bash
CraziiEmu --debug-server "/path/to/eboot.bin"            # 127.0.0.1:5714
CraziiEmu --debug-server=0.0.0.0:5714 "/path/to/eboot.bin"
```
...
```bash
./tools/CraziiEmu.DebuggerFrontend/run.sh
```

It connects to `127.0.0.1:5714` and opens `http://127.0.0.1:8765/` by default.
See [`tools/CraziiEmu.DebuggerFrontend/README.md`](../tools/CraziiEmu.DebuggerFrontend/README.md)
